<template>
  <div class="beacon-app" :class="{ 'focus-mode': focusMode }">
    ...
    <main class="site-main">
      <section v-if="route.name === 'landing'" class="landing-page">
        <div v-if="!focusMode" class="marquee-rail site-shell" aria-label="Beacon release notices">
          <div class="marquee-window" aria-hidden="true">
            <div class="marquee-track">
              <span>LIVE V6 COLLATERAL PASSPORT</span><i></i><span>FINALIZED ON TESTNET BRADBURY</span><i></i><span>COLLATERAL POLICY IS DETERMINISTIC</span><i></i><span>INDEPENDENT VALIDATOR EVIDENCE</span><i></i><span>V6 IDENTITY-BOUND CONSENSUS</span><i></i>
              <span>LIVE V6 COLLATERAL PASSPORT</span><i></i><span>FINALIZED ON TESTNET BRADBURY</span><i></i><span>COLLATERAL POLICY IS DETERMINISTIC</span><i></i><span>INDEPENDENT VALIDATOR EVIDENCE</span><i></i><span>V6 IDENTITY-BOUND CONSENSUS</span><i></i>
            </div>
          </div>
        ...
        <div class="site-shell proof-strip" aria-label="Beacon release proof summary">
          <div class="proof-metric"><strong>V6</strong><span>IDENTITY-BOUND PROOF</span></div>
        ...
        <div class="site-shell landing-registry">
          ...
          <div v-if="!configured" class="state-block">Configure the V6 contract to surface live collateral records.</div>
      ...
      <section v-else-if="route.name === 'registry'" class="page-shell">
        ...
        <div v-if="!configured" class="notice warning">V6 contract configuration is missing. No illustrative assets are shown.</div>
        <template v-else>
      ...
      <section v-else-if="route.name === 'detail'" class="page-shell">
        ...
        <div v-if="!configured" class="notice warning">V6 contract configuration is missing. No passport is fabricated.</div>
    ...
    <footer v-if="!focusMode" class="site-footer"><div class="site-shell footer-grid"><span>BEACON / COLLATERAL ADMISSION</span><span>V6 / BRADBURY / PUBLIC READS</span><span class="mono">F TO FOCUS</span></div></footer>
...
import { V6_RELEASE_PROOF } from "./services/releaseProof.js";
...
const releaseProof = V6_RELEASE_PROOF;
