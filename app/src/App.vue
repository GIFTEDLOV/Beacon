<template>
  <div class="beacon-app" :class="{ 'focus-mode': focusMode }">
    ...
    <main class="site-main">
      <section v-if="route.name === 'landing'" class="landing-page">
        <div v-if="!focusMode" class="marquee-rail site-shell" aria-label="Beacon release notices">
          <div class="marquee-window" aria-hidden="true">
            <div class="marquee-track">
              <span>LIVE V4 COLLATERAL PASSPORT</span><i></i><span>FINALIZED ON TESTNET BRADBURY</span><i></i><span>COLLATERAL POLICY IS DETERMINISTIC</span><i></i><span>INDEPENDENT VALIDATOR EVIDENCE</span><i></i><span>V4 CONSENSUS LIVE-PROVEN</span><i></i>
              <span>LIVE V4 COLLATERAL PASSPORT</span><i></i><span>FINALIZED ON TESTNET BRADBURY</span><i></i><span>COLLATERAL POLICY IS DETERMINISTIC</span><i></i><span>INDEPENDENT VALIDATOR EVIDENCE</span><i></i><span>V4 CONSENSUS LIVE-PROVEN</span><i></i>
            </div>
          </div>
        ...
        <div class="site-shell proof-strip" aria-label="Beacon release proof summary">
          <div class="proof-metric"><strong>V4</strong><span>FINALIZED PROOF</span></div>
        ...
        <div class="site-shell landing-registry">
          ...
          <div v-if="!configured" class="state-block">Configure the V4 contract to surface live collateral records.</div>
      ...
      <section v-else-if="route.name === 'registry'" class="page-shell">
        ...
        <div v-if="!configured" class="notice warning">V4 contract configuration is missing. No illustrative assets are shown.</div>
        <template v-else>
      ...
      <section v-else-if="route.name === 'detail'" class="page-shell">
        ...
        <div v-if="!configured" class="notice warning">V4 contract configuration is missing. No passport is fabricated.</div>
        ...
        <template v-else>
          ...
          <div class="detail-grid"><section class="data-section"><div class="section-heading block-heading"><div><span class="section-index">04 / OBJECTIVE</span><h2>Market evidence</h2></div><span class="source-tag validator">VALIDATOR-DERIVED</span></div><dl class="data-list"><div><dt>Coverage</dt><dd>{{ detailPassport?.objective_coverage || '—' }}</dd></div><div><dt>Primary status</dt><dd>{{ detailPassport?.primary_source_status || '—' }}</dd></div><div><dt>Secondary status</dt><dd>{{ detailPassport?.secondary_source_status || '—' }}</dd></div><div><dt>Peg deviation</dt><dd>{{ detailPassport?.peg_deviation_bps ?? '—' }} BPS</dd></div><div><dt>Secondary deviation</dt><dd>{{ detailPassport?.secondary_peg_deviation_bps ?? '—' }} BPS</dd></div><div><dt>Market marker</dt><dd>{{ detailPassport?.market_timestamp || '—' }}</dd></div></dl></section><section class="data-section"><div class="section-heading block-heading"><div><span class="section-index">05 / POLICY TRACE</span><h2>Deterministic output</h2></div><span class="source-tag on-chain">ON-CHAIN</span></div><dl class="data-list"><div><dt>Safety cap</dt><dd>{{ detailPassport?.safety_cap || '—' }}</dd></div><div><dt>Policy basis</dt><dd>{{ detailPassport?.policy_basis || '—' }}</dd></div><div><dt>Failure state</dt><dd>{{ detailPassport?.failure_state || 'NOT_EVALUATED' }}</dd></div><div><dt>Challenge trigger</dt><dd class="break-value">{{ detailPassport?.trigger_challenge_id || 'INITIAL EVALUATION' }}</dd></div></dl></section></div>
          ...
          <section class="section-block"><div class="section-heading block-heading"><div><span class="section-index">07 / EVIDENCE SOURCES</span><h2>Submitted source set</h2></div><span class="source-tag external">UNTRUSTED INPUT</span></div><div class="evidence-list"><a v-for="source in evidenceSources" :key="source.label" :href="source.url" target="_blank" rel="noreferrer noopener"><span class="evidence-role">{{ source.label }}</span><code>{{ source.url }}</code><span aria-hidden="true">↗</span></a></div></section>

          <div class="detail-grid"><section class="data-section"><div class="section-heading block-heading"><div><span class="section-index">08 / PASSPORT HISTORY</span><h2>Immutable versions</h2></div><span class="mono">{{ historyRows.length }} RECORD{{ historyRows.length === 1 ? '' : 'S' }}</span></div><div v-if="historyRows.length" class="history-list"><div v-for="passport in historyRows" :key="passport.version" class="history-row"><span class="history-version">V{{ passport.version }}</span><span class="history-verdict" :class="riskClass(passport.verdict)">{{ passport.verdict || passport.failure_state }}</span><span>{{ passport.max_ltv_bps }} BPS</span><small>{{ passport.evaluated_at || passport.market_timestamp || 'MARKER UNAVAILABLE' }}</small></div></div><div v-else class="state-block">No passport history is available.</div></section><section class="data-section"><div class="section-heading block-heading"><div><span class="section-index">09 / CHALLENGES</span><h2>Governance record</h2></div><span class="mono">{{ challengeRows.length }} RECORD{{ challengeRows.length === 1 ? '' : 'S' }}</span></div><div v-if="challengeRows.length" class="history-list"><div v-for="challenge in challengeRows" :key="challenge.challenge_id" class="history-row challenge-history"><span class="history-version">{{ challenge.category }}</span><span>{{ challenge.reason }}</span><small>V{{ challenge.target_version }} / {{ challenge.status }}</small></div></div><div v-else class="state-block">No challenges recorded.</div></section></div>
    ...
    <footer v-if="!focusMode" class="site-footer"><div class="site-shell footer-grid"><span>BEACON / COLLATERAL ADMISSION</span><span>V4 / BRADBURY / PUBLIC READS</span><span class="mono">F TO FOCUS</span></div></footer>
...
import { V4_RELEASE_PROOF } from "./services/releaseProof.js";
...
const releaseProof = V4_RELEASE_PROOF;
...
const evidenceSources = computed(() => detailAsset.value ? [
  { label: "ISSUER", url: detailAsset.value.issuer_url },
  { label: "REDEMPTION", url: detailAsset.value.redemption_url },
  { label: "BACKING", url: detailAsset.value.reserve_backing_url },
  { label: "SECURITY", url: detailAsset.value.security_url },
  { label: "GOVERNANCE", url: detailAsset.value.governance_url }
] : []);
