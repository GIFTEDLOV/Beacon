    ...
    <main class="site-main">
      <section v-if="route.name === 'landing'" class="landing-page">
        ...
        <div class="site-shell hero-layout">
          ...
          <div class="hero-context">
            ...
            <div class="context-row"><span>03</span><span>CONSENSUS Ã¢â€ â€™ POLICY Ã¢â€ â€™ PASSPORT</span></div>
          </div>
        </div>

        <div class="site-shell hero-actions-row">
          <div class="hero-actions"><button class="button button-accent" @click="navigate('/assets')">Explore collateral <span aria-hidden="true">Ã¢â€ â€”</span></button><button class="button button-line" @click="navigate('/submit')">Submit asset <span aria-hidden="true">Ã¢â€ â€”</span></button></div>
          <span class="mono muted">F / FOCUS MODE</span>
        </div>

        <div class="site-shell proof-strip" aria-label="Beacon release proof summary">
          <div class="proof-metric"><strong>V8</strong><span>IDENTITY-BOUND PROOF</span></div>
          ...
          <div class="proof-metric"><strong>{{ proofPassport?.verdict || 'Ã¢â‚¬â€' }}</strong><span>PASSPORT V1 Ã‚Â· {{ proofPassport?.max_ltv_bps ?? 'Ã¢â‚¬â€' }} BPS</span></div>
        </div>

        <div class="site-shell landing-registry">
          <div class="section-heading compact-heading"><div><span class="section-index">01 / REGISTRY PREVIEW</span><h2>Collateral, under scrutiny.</h2></div><button class="text-link" @click="navigate('/assets')">Open registry <span aria-hidden="true">Ã¢â€ â€”</span></button></div>
          <div v-if="!configured" class="state-block">Configure the V8 contract to surface live collateral records.</div>
          ...
          <div v-else class="preview-list"><button v-for="item in assets.slice(0, 3)" :key="item.asset_id" class="preview-row" @click="openDetail(item.asset_id)"><span class="preview-identity"><strong>{{ item.name }}</strong><small>{{ item.symbol }} / {{ item.chain }}</small></span><span class="mono">{{ displayState(item.passport).label }}</span><strong class="preview-ltv">{{ item.passport?.max_ltv_bps ?? 'Ã¢â‚¬â€' }} <small>BPS</small></strong><span aria-hidden="true" class="arrow">Ã¢â€ â€”</span></button></div>
        </div>
      ...
      <section v-else-if="route.name === 'registry'" class="page-shell">
        ...
        <template v-else>
          ...
          <div v-else class="registry-table-wrap"><table class="registry-table"><caption class="sr-only">Beacon collateral registry</caption><thead><tr><th>Asset</th><th>Network</th><th>Status</th><th>Passport</th><th>Verdict</th><th>Max LTV</th><th>Peg</th><th>Liquidity</th><th>Confidence</th><th>Updated</th></tr></thead><tbody><tr v-for="item in filteredAssets" :key="item.asset_id" tabindex="0" @click="openDetail(item.asset_id)" @keydown.enter="openDetail(item.asset_id)"><td><button class="table-link" @click.stop="openDetail(item.asset_id)"><strong>{{ item.name }}</strong><small>{{ item.symbol }} / {{ item.asset_id }}</small></button></td><td><span>{{ item.chain }}</span><small class="address">{{ item.token_address }}</small></td><td><span class="mono status-text">{{ item.lifecycle_status || 'Ã¢â‚¬â€' }}</span></td><td><span class="mono">V{{ item.passport?.version || 0 }}</span></td><td><span class="verdict-text" :class="riskClass(displayState(item.passport).label)">{{ displayState(item.passport).label }}</span></td><td class="ltv-cell">{{ item.passport?.max_ltv_bps ?? 0 }}<small>BPS</small></td><td>{{ item.passport?.peg_risk || 'Ã¢â‚¬â€' }}</td><td>{{ item.passport?.liquidity_risk || 'Ã¢â‚¬â€' }}</td><td>{{ item.passport?.confidence || 'Ã¢â‚¬â€' }}</td><td>{{ item.passport?.evaluated_at || item.passport?.market_timestamp || 'Ã¢â‚¬â€' }}</td></tr></tbody></table></div>
        </template>
      </section>

      <section v-else-if="route.name === 'detail'" class="page-shell">
        <button class="back-link" @click="navigate('/assets')">Ã¢â€ Â Back to registry</button>
        <div v-if="!configured" class="notice warning">V8 contract configuration is missing. No passport is fabricated.</div>
        ...
        <template v-else>
          ...
          <section class="section-block"><div class="section-heading block-heading"><div><span class="section-index">04 / IDENTITY</span><h2>Authenticated asset binding</h2></div><span class="source-tag validator">{{ identityDisplay.status }}</span></div><div class="identity-grid"><div><span class="section-index">CANONICAL CHAIN</span><code>{{ identityDisplay.canonicalChain }}</code></div><div><span class="section-index">NAMESPACE</span><code>{{ identityDisplay.canonicalNamespace }}</code></div><div><span class="section-index">TOKEN ADDRESS</span><code>{{ identityDisplay.canonicalAddress }}</code></div><div><span class="section-index">AUTHENTICATED NAME / SYMBOL</span><strong>{{ identityDisplay.canonicalName }} / {{ identityDisplay.canonicalSymbol }}</strong></div><div><span class="section-index">COINGECKO ID</span><code>{{ identityDisplay.coingeckoId }}</code><small>{{ identityDisplay.coingeckoBinding }}</small></div><div><span class="section-index">COINPAPRIKA ID</span><code>{{ identityDisplay.coinpaprikaId }}</code><small>{{ identityDisplay.coinpaprikaBinding }}</small></div><div><span class="section-index">ISSUER AUTHORITY</span><strong>{{ detailPassport?.official_issuer_domain || 'UNVERIFIED' }}</strong></div><div><span class="section-index">IDENTITY DIGEST</span><CopyHash :value="detailPassport?.identity_digest || 'Ã¢â‚¬â€'" label="Copy identity digest" /></div><div><span class="section-index">SUBMITTED CLAIM</span><code>{{ identityDisplay.submittedChain }} / {{ identityDisplay.submittedAddress }}</code><small>UNVERIFIED UNTIL DUAL PROVIDER BINDING</small></div></div></section>
          <div class="detail-grid"><section class="data-section"><div class="section-heading block-heading"><div><span class="section-index">05 / OBJECTIVE</span><h2>Address-bound market evidence</h2></div><span class="source-tag validator">VALIDATOR-DERIVED</span></div><dl class="data-list"><div><dt>Coverage</dt><dd>{{ detailPassport?.objective_coverage || 'Ã¢â‚¬â€' }}</dd></div><div><dt>Primary status</dt><dd>{{ detailPassport?.primary_source_status || 'Ã¢â‚¬â€' }}</dd></div><div><dt>Secondary status</dt><dd>{{ detailPassport?.secondary_source_status || 'Ã¢â‚¬â€' }}</dd></div><div><dt>Peg deviation</dt><dd>{{ detailPassport?.peg_deviation_bps ?? 'Ã¢â‚¬â€' }} BPS</dd></div><div><dt>Secondary deviation</dt><dd>{{ detailPassport?.secondary_peg_deviation_bps ?? 'Ã¢â‚¬â€' }} BPS</dd></div><div><dt>Market marker</dt><dd>{{ detailPassport?.market_timestamp || 'Ã¢â‚¬â€' }}</dd></div></dl></section><section class="data-section"><div class="section-heading block-heading"><div><span class="section-index">06 / POLICY TRACE</span><h2>Deterministic output</h2></div><span class="source-tag on-chain">ON-CHAIN</span></div><dl class="data-list"><div><dt>Safety cap</dt><dd>{{ detailPassport?.safety_cap || 'Ã¢â‚¬â€' }}</dd></div><div><dt>Policy basis</dt><dd>{{ detailPassport?.policy_basis || 'Ã¢â‚¬â€' }}</dd></div><div><dt>Failure state</dt><dd>{{ detailPassport?.failure_state || 'NOT_EVALUATED' }}</dd></div><div><dt>Challenge set</dt><dd>{{ detailPassport?.challenge_count || 0 }} evaluated / {{ detailPassport?.supported_challenge_count || 0 }} supported</dd></div><div><dt>Challenge-set digest</dt><dd class="break-value">{{ detailPassport?.challenge_set_digest || 'INITIAL EVALUATION' }}</dd></div></dl></section></div>
...
import { exactLiveProofState, filterRegistry, identityDisplayState, parseBeaconPath, passportDisplayState, validateChallengeInput, validateSubmissionFields } from "./services/presentation.js";
import { V8_RELEASE_PROOF } from "./services/releaseProof.js";
...
const releaseProof = V8_RELEASE_PROOF;
const liveProofState = computed(() => exactLiveProofState(assets.value, releaseProof.assetId));
const proofAsset = computed(() => liveProofState.value.asset);
const proofPassport = computed(() => liveProofState.value.passport);
const proofLiveAvailable = computed(() => liveProofState.value.available);
...
const semanticCheckpoints = [
  ...
  { key: "BACKING", role: "BACKING", label: "Backing" },
...
const primaryRiskFields = [
  ...
  { key: "governance_risk", label: "GOVERNANCE", provenance: "SEMANTIC / BOUNDED" },
...
const identityDisplay = computed(() => identityDisplayState(detailPassport.value || {}, detailAsset.value || {}, checkpointState.value || {}));
...
async function loadDetail(id) {
  ...
  try {
    const [asset, passport, history, challenges, checkpoints] = await Promise.all([
      registry.asset(id),
      registry.currentPassport(id),
      registry.passportHistory(id),
      registry.challengeRecords(id),
      registry.checkpointState(id)
    ]);
    checkpointState.value = checkpoints;
    detailAsset.value = Object.keys(asset || {}).length ? asset : null;
    detailPassport.value = registry.decoratePassport(passport, checkpoints);
