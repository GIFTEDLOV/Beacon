...
def _challenge_evidence(url,i,category):
 if not _is_https_source(url):
  return{_K0:SOURCE_IDENTITY_UNVERIFIED,_K1:_K3,"text":"",_K2:""}
 try:
  w=gl.nondet.web.get(url)
  status=_status_code(w)
  if status>=500 or status==429:
   return{_K0:EVIDENCE_UNAVAILABLE}
  if 300<=status<400 or not _response_host_matches(w,url):
   return{_K0:SOURCE_IDENTITY_UNVERIFIED,_K1:_K3,"text":"",_K2:""}
  if status>=400:
   return{_K1:_K3,"text":"",_K2:""}
  try:
   y=_body_text(w)
  except (UnicodeError,TypeError):
   return{_K0:INVALID_SOURCE,_K1:_K3,"text":"",_K2:""}
  if not y.strip()or len(y)>MAX_RESPONSE_LENGTH:
   return{_K1:_K3,"text":"",_K2:""}
  ...
  return{_K1:_K48,"text":text,_K2:digest,}
 except Exception:
  return{_K0:EVIDENCE_UNAVAILABLE}
...
def _challenge_validator(z,lr):
 try:
  if not isinstance(lr,gl.vm.Return)or not isinstance(lr.calldata,dict):
   return False
  p=lr.calldata
  i,target_version,category,reason,evidence_url=z
  e=_challenge_evidence(evidence_url,i,category)
...
def _run_challenge(i,c):
 z=(i,c.target_version,c.category,c.reason,c.evidence_url)
 try:
...
class Beacon(gl.Contract):
 ...
 def reassess_asset(self,asset_id:str)->None:
  ...
  for c in e:
   ...
   if _K0 in r or not _valid_challenge_result({key:r.get(key)for key in(_K7,_K5)})or not r.get(_K2):
