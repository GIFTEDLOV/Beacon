...
def _fresh(x):
    ...
    return t>=0 and n>=0 and t<=n+300 and n-t<=MAX_AGE
def _host(x): return x[8:].split("/",1)[0].split("?",1)[0].split("#",1)[0].lower() if isinstance(x,str) and x.startswith("https://") else ""
def _private_host(h):
    if h in ("localhost","0.0.0.0","255.255.255.255") or ":" in h:return True
    p=h.split(".")
    if len(p)!=4 or not all(x.isdigit() for x in p):return False
    a,b,c,d=(int(x) for x in p)
    return a==0 or a==10 or a==127 or a>=224 or (a==100 and 64<=b<=127) or (a==169 and b==254) or (a==172 and 16<=b<=31) or (a==192 and b in (0,168)) or (a==198 and b in (18,19)) or (a==203 and b==0)
def _https(x):
    if not isinstance(x,str) or not re.fullmatch(r"https://[A-Za-z0-9.-]+(?:/[A-Za-z0-9._~:/?#\[\]@!$&'()*+,;=%-]*)?",x):return False
    h=_host(x);return bool(h) and "." in h and not h.endswith(".") and not _private_host(h)
def _same_host(response,url):
    try:
        ...
        return final is None or str(final)==url
...
def _fetch(url,limit,as_json=False):
    ...
    try:
        ...
        if not as_json:return OK,text
        try:return OK,json.loads(text)
        except Exception:return INVALID,None
    except Exception:return UNAVAILABLE,None
def _fixed(x):
...
def _identity_once(provider,address,cgid,cpid):
    ...
    if status!=OK or not isinstance(data,dict):
        r=_empty(provider,expected);r["source_status"]=status;return r
    if provider==CG:
        ...
        return {"provider":provider,"source_status":OK if valid else INVALID,"provider_id":data.get("id",expected).lower() if isinstance(data.get("id"),str) else expected,"canonical_address":exact.lower() if isinstance(exact,str) else "","canonical_symbol":data.get("symbol","").upper() if isinstance(data.get("symbol"),str) else "","canonical_name":data.get("name","").strip() if isinstance(data.get("name"),str) else "","binding_status":VERIFIED if valid else UNVERIFIED}
    contracts=data.get("contracts");valid=False;found=""
    if isinstance(contracts,list):
        for i in contracts:
            if isinstance(i,dict) and str(i.get("platform","")).lower()=="eth-ethereum" and isinstance(i.get("contract"),str):
                found=i["contract"].lower()
                if found==address.lower():valid=True;break
    ...
    return {"provider":provider,"source_status":OK if valid else status if status!=OK else INVALID,"provider_id":data.get("id",expected).lower() if isinstance(data.get("id"),str) else expected,"canonical_address":found,"canonical_symbol":data.get("symbol","").upper() if isinstance(data.get("symbol"),str) else "","canonical_name":data.get("name","").strip() if isinstance(data.get("name"),str) else "","binding_status":VERIFIED if valid else UNVERIFIED}
...
def _derived(asset,identities):
    ...
    good=good and g.get("source_status")==OK and p.get("source_status")==OK and isinstance(g.get("provider_id"),str) and isinstance(p.get("provider_id"),str) and isinstance(g.get("canonical_address"),str) and isinstance(p.get("canonical_address"),str) and isinstance(g.get("canonical_symbol"),str) and isinstance(p.get("canonical_symbol"),str) and isinstance(g.get("canonical_name"),str) and isinstance(p.get("canonical_name"),str)
...
def _sem_once(role,url,asset):
    ...
    try:output=_sem_output(_resolve(gl.nondet.exec_prompt(prompt,response_format="json")),role)
...
def _challenge_facts(url,body,asset,category):
    ...
    if category=="LIQUIDITY" and host=="api.dexscreener.com" and "/latest/dex/pairs/ethereum/" in url:
        ...
        metrics=isinstance(l,dict) and isinstance(v,dict) and ("usd" in l or "base" in l) and "h24" in v;exact=isinstance(b,dict) and str(b.get("address","")).lower()==address;pair=str(p.get("pairAddress","")).lower() if isinstance(p,dict) else "";requested=url.rsplit("/",1)[-1].split("?",1)[0].lower()
        return {"authority":host,"namespace":NS,"address":address,"category":category,"asset_binding":True,"pair_address":pair,"liquidity":True,"volume":True} if isinstance(p,dict) and str(p.get("chainId","")).lower()==ETH and metrics and exact and pair==requested else None
...
def _challenge_output(x):
    ...
    if not isinstance(x,dict) or set(x)!={"evaluation_result","evaluation_reason_code"}:return None
    result=x.get("evaluation_result");code=x.get("evaluation_reason_code")
...
class Beacon(gl.Contract):
    ...
    def _asset(self,aid):
        ...
        if not isinstance(x,str):_fail("corrupt asset")
    ...
    def _submission(self,chain,address,currency,cgid,cpid,urls):
        if not isinstance(chain,str) or chain.strip().lower() not in (NS,ETH,"eth","mainnet"): _fail("unsupported chain")
        ...
        if not isinstance(currency,str) or currency.strip().upper()!=USD:_fail("unsupported target currency")
        for x in (cgid,cpid):
    ...
    def _store_identity(self,a,p,result):
        claim=a["coingecko_id_claim"] if p==CG else a["coinpaprika_id_claim"];w=_id_witness(result) or {};record=dict(w);record.update({"asset_id":a["asset_id"],"canonical_namespace":NS,"canonical_token_address":a["token_address"],"canonical_fact_digest":_digest(w),"verified_at":_now()});self.identities[self._key(a["asset_id"],"identity",p)]=_json(record);d=self._derived(a);a.update({"identity_status":d["identity_status"],"canonical_name":d["canonical_name"],"canonical_symbol":d["canonical_symbol"]});self._save_asset(a)
    def _store_semantic(self,a,role,r):
        r=dict(r);r.update({"asset_id":a["asset_id"],"role":role,"source_url":URLS[role],"identity_digest":self._derived(a)["identity_digest"],"verified_at":_now(),"version":a["current_version"]});self.semantics[self._key(a["asset_id"],"semantic",role)]=_json(r)
    ...
    def _store_market(self,a,p,r):
        d=self._derived(a);r=dict(r);r.update({"asset_id":a["asset_id"],"provider":p,"provider_id":d["coingecko_id"] if p==CG else d["coinpaprika_id"],"canonical_namespace":NS,"canonical_token_address":a["token_address"],"canonical_symbol":d["canonical_symbol"],"identity_digest":d["identity_digest"],"observed_at":_now(),"version":a["current_version"]});self.markets[self._key(a["asset_id"],"market",p)]=_json(r)
    ...
    def _build(self,a,version,items):
        ...
        for role in ROLES:
            ...
            if not isinstance(x,dict) or x.get("asset_id")!=a["asset_id"] or x.get("role")!=role or x.get("source_url")!=URLS[role] or x.get("identity_digest")!=d["identity_digest"] or x.get("source_status")!=OK or x.get("authority_status")!=VERIFIED or x.get("asset_binding_status")!=VERIFIED or x.get("risk") not in RISKS or int(x.get("version",0))>int(a["current_version"]):_fail("semantic checkpoints incomplete")
            sem[role]=x
        for p in (CG,CP):
            x=self._read(self.markets,self._key(a["asset_id"],"market",p))
            expected_id=d["coingecko_id"] if p==CG else d["coinpaprika_id"]
            if not isinstance(x,dict) or x.get("asset_id")!=a["asset_id"] or x.get("provider")!=p or x.get("provider_id")!=expected_id or x.get("canonical_namespace")!=NS or x.get("canonical_token_address")!=a["token_address"] or x.get("canonical_symbol")!=d["canonical_symbol"] or x.get("identity_digest")!=d["identity_digest"] or x.get("source_status")!=OK or x.get("peg_risk") not in RISKS or x.get("liquidity_risk") not in RISKS or not _fresh(x.get("market_timestamp","")):_fail("market checkpoints incomplete")
            m[p]=x
    ...
    def challenge_asset(self,asset_id,target_version,category,reason,evidence_url):
        ...
        if not reason or len(reason)>MAX_REASON or not isinstance(evidence_url,str) or len(evidence_url)>MAX_URL or not _https(evidence_url) or self._derived(a)["identity_status"]!=VERIFIED:_fail("invalid challenge")
        ...
        challenger=gl.message.sender_address.as_hex;reason_digest=_digest({"reason":reason});cid=asset_id+"#"+str(int(target_version))+"#"+category+"#"+challenger.lower()+"#"+reason_digest+"#"+evidence["evidence_digest"]
        if self.challenges.get(cid) is not None:_fail("duplicate challenge")
        c={"challenge_id":cid,"asset_id":asset_id,"target_version":target_version,"challenger":challenger,"category":category,"reason":reason,"reason_digest":reason_digest,"evidence_url":evidence_url,"evidence_digest":evidence["evidence_digest"],"bounded_evidence_excerpt":evidence["bounded_evidence_excerpt"],"status":OPEN,"evaluation_result":"","evaluation_reason_code":"","resolution_version":0};self.challenges[cid]=_json(c);ids=self.challenge_ids.get_or_insert_default(asset_id);ids.append(cid);a["lifecycle_status"]=CHALLENGED;self._save_asset(a);return cid
    def _stored_valid(self,c,a):
        try:f=json.loads(c["bounded_evidence_excerpt"])
        except Exception:return False
        expected_provider=a["coinpaprika_id_claim"] if c.get("category")=="OTHER" else ""
        return c.get("status")==OPEN and c.get("asset_id")==a["asset_id"] and c.get("target_version")==a["current_version"] and c.get("category") in CATEGORIES and bool(c.get("reason")) and len(c["reason"])<=MAX_REASON and isinstance(c.get("evidence_url"),str) and len(c["evidence_url"])<=MAX_URL and _https(c.get("evidence_url")) and c.get("reason_digest")==_digest({"reason":c["reason"]}) and c.get("evidence_digest")==_digest(f) and f.get("authority")==_host(c.get("evidence_url")) and f.get("asset_binding") is True and f.get("namespace")==NS and isinstance(f.get("address"),str) and f.get("address","").lower()==a["token_address"] and f.get("category")==c["category"] and (not expected_provider or f.get("provider_id")==expected_provider)
    @gl.public.write
    ...
    @gl.public.view
    def asset(self,asset_id)->dict:
        return self._asset(asset_id) if self.assets_store.get(asset_id) is not None else {}
    @gl.public.view
